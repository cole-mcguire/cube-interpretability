...
\begin{enumerate}
  ...
  \item \textbf{Improved representations.}  The automated evaluation found that all
        state-based representations score uniformly zero.  Future work could test
        whether augmenting \texttt{piece\_identity} with explicit move-effect
        annotations (e.g., listing which pieces are in the U layer before and after
        each move) allows a model to simulate individual moves reliably enough to
        solve $d{=}3$ cases.  Alternatively, a hybrid representation that provides
        both the state and a partial BFS hint (e.g., a single ``next best move''
        derived from the distance table) may offer a path to non-zero state-based
        solve rates.
